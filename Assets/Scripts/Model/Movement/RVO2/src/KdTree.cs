...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            {
                agentTree_[node].maxX_ = TSMath.Max(agentTree_[node].maxX_, agents_[i].position_.x);
                agentTree_[node].minX_ = TSMath.Min(agentTree_[node].minX_, agents_[i].position_.x);
                agentTree_[node].maxY_ = TSMath.Max(agentTree_[node].maxY_, agents_[i].position_.y);
                agentTree_[node].minY_ = TSMath.Min(agentTree_[node].minY_, agents_[i].position_.y);
            }
            ...
            if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                FP splitValue = 0.5f * (isVertical ? agentTree_[node].maxX_ + agentTree_[node].minX_ : agentTree_[node].maxY_ + agentTree_[node].minY_);
        ...
        private ObstacleTreeNode buildObstacleTreeRecursive(IList<Obstacle> obstacles)
        {
            ...
            for (int i = 0; i < obstacles.Count; ++i)
            {
                ...
                for (int j = 0; j < obstacles.Count; ++j)
                {
                    ...
                    if (new FloatPair(TSMath.Max(leftSize, rightSize), TSMath.Min(leftSize, rightSize)) >= new FloatPair(TSMath.Max(minLeft, minRight), TSMath.Min(minLeft, minRight)))
                    {
                        break;
                    }
                }
        ...
        private void queryAgentTreeRecursive(Agent agent, ref FP rangeSq, int node)
        {
            ...
            {
                FP distSqLeft = RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minX_ - agent.position_.x)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].left_].maxX_)) + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minY_ - agent.position_.y)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].left_].maxY_));
                FP distSqRight = RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minX_ - agent.position_.x)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x - agentTree_[agentTree_[node].right_].maxX_)) + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minY_ - agent.position_.y)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y - agentTree_[agentTree_[node].right_].maxY_));
        ...
        private void queryObstacleTreeRecursive(Agent agent, FP rangeSq, ObstacleTreeNode node)
        {
            if (node != null)
            {
                ...
                FP agentLeftOfLine = RVOMath.leftOf(obstacle1.point_, obstacle2.point_, agent.position_);
        ...
        private bool queryVisibilityRecursive(TSVector2 q1, TSVector2 q2, FP radius, ObstacleTreeNode node)
        {
            ...
            FP invLengthI = 1.0f / RVOMath.absSq(obstacle2.point_ - obstacle1.point_);
            ...
            FP invLengthQ = 1.0f / RVOMath.absSq(q2 - q1);

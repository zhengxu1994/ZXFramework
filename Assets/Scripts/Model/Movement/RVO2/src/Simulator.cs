...
namespace RVO
{
    ...
    public class Simulator
    {
        ...
        public FP doStep()
        {
            if (workers_ == null)
            {
                ...
                for (int block = 0; block < workers_.Length; ++block)
                {
                    ...
                    workers_[block] =
                        new Worker(block * getNumAgents() / workers_.Length,
                            (block + 1) * getNumAgents() / workers_.Length,
                            doneEvents_[block]);
                }
            }
         ...
         */
        public bool
        queryVisibility(TSVector2 point1, TSVector2 point2, FP radius)
        {
